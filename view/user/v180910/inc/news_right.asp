<div class="area_right">
		...
		<div class="line_right" style="border-top:2px soild #000; border-left:1px solid #d0d0d0; border-right:1px solid #d0d0d0; border-bottom:1px solid #d0d0d0;">
				...
				<%
					If Ubound( right_card_news ) > -1 Then 
				%>
				<div class="area_right_photobox" style="border:none; border-top:1px solid #d0d0d0; margin-bottom:0;">
					...
					<%
						For loop_cnt = 0 To Ubound( right_card_news )
							side_article_id = right_card_news(loop_cnt).item("ArticleID")
							side_catr_code = right_card_news(loop_cnt).item("CatrCode")
							side_img_path = right_card_news(loop_cnt).item("img_path")
							side_subject = right_card_news(loop_cnt).item("Subject")
							side_contents = right_card_news(loop_cnt).item("Contents")
							img_info_main = right_card_news(loop_cnt).item("img_info_main")
								
							If img_info_main <> "" Then 
								side_img_path = img_info_main
							End If 
					%>
						<div style="height: 220px; ">
							<div class="area_right_photo" style="height:160px;overflow:hidden" >
								<a href="/news/view?ArticleID=<%=side_article_id%>&catr=1701">
									<img src="<%=side_img_path%>" alt="<%=side_subject%>" style="width:300px;" />
								</a>
							</div>
