...
function init_loading_dom() {
    ...
    loading_html += "<img src=\"http://img.healthi.kr/img/loading/loading.jpg\" style=\"width:64px;border-radius:100%\" >";
    loading_html += "</div>";
...
function share_sns(type, share_title, img_path, share_url ) {
    var page_url;
	...
	page_url = encodeURIComponent( share_url );
...
보육예약 서치 레이어 handler
**********************************************************************************/
function ReservChild_box( arg_type ) {
    ...
    if( arg_type == "open" ) {
        ...
        option = {
            "showType" : "ReservChild_box"
            ,"showID" : "ReservChild_box"
            ,"onBlock" : ""
            ,"onOverlayClick" : ""
        }
        show_blockUI(option);
        ...
        jQuery.unblockUI("ReservChild_box");    
    }
...
function popupOpen( path, popup_name, width, height ) { 
	window.open( path, popup_name, "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height="+ height +",width="+ width );	
}
